...
function addCartModal(id, name, price) {
    ...
    if (exist) exist.qty += 1; else CART.push({ id, name, price, qty: 1, cat });
...
function renderOCList() {
    ...
    const grouped = {};
    STATE.logs.filter(l => l.tipo === 'oc_pendente' || l.tipo === 'compra').forEach(l => {
        if (!grouped[l.id]) grouped[l.id] = { id: l.id, obra_id: l.obra_id, forn_id: l.fornecedor_id, tipo: l.tipo, fase: l.fase_obra, data: l.data, itemsText: [], total: 0 };
        grouped[l.id].itemsText.push(l.produto_nome);
        grouped[l.id].total += parseFloat(l.valor_total || 0);
    });

    let arr = Object.values(grouped).sort((a, b) => parseInt(b.id) - parseInt(a.id));
        ...
        <tr class="border-b hover:bg-slate-50 transition">
            ...
            <td class="p-4 text-center">
                <div class="flex justify-center gap-2">
                    ...
                    ` : `
                        <button onclick="estornarCompra('${o.id}')" class="p-2 border border-orange-300 text-orange-600 hover:bg-orange-50 rounded" title="Estornar Confirmação"><i data-lucide="rotate-ccw" width="14"></i></button>
                    `}
                    ...
                    <button onclick="printRomaneio('${o.id}')" class="p-2 border border-teal-200 text-teal-700 hover:bg-teal-50 rounded bg-teal-50/50" title="Romaneio"><i data-lucide="truck" width="14"></i></button>
...
async function estornarCompra(id) {
    if (!confirm("Deseja estornar a confirmação desta O.C.? Ela voltará a ser Pendente.")) return;
    showLoading(true);
    const { error } = await sb.from('jsp_logs').update({ tipo: 'oc_pendente', status_financeiro: 'PENDENTE' }).eq('id', id).eq('tipo', 'compra');
    if (error) { showLoading(false); return showToast("Erro: " + error.message, true); }
...
function printOC(id) {
    ...
    if (!logsOC.length) return showToast("O.C. não encontrada.", true);
    ...
    let fullObs = o.observacao || 'Nenhuma observação.';
    let justObs = fullObs;
    if (fullObs.includes(' | Obs: ')) {
        const parts = fullObs.split(' | Obs: ');
        justObs = parts[1];
    ...
    let total = 0;
    const linhas = logsOC.map(i => {
        const sub = parseFloat(i.valor_total);
        total += sub;
        return `<tr>
            <td style="padding: 8px; border-bottom: 1px solid #000;">${i.produto_nome}</td>
            <td style="padding: 8px; border-bottom: 1px solid #000; text-align: center;">${i.quantidade}</td>
            <td style="padding: 8px; border-bottom: 1px solid #000; text-align: right;">${formatMoney(parseFloat(i.valor_total) / parseFloat(i.quantidade))}</td>
            <td style="padding: 8px; border-bottom: 1px solid #000; text-align: right;">${formatMoney(sub)}</td>
        </tr>`;
    }).join('');
    ...
    const html = `<div style="font-family: Arial, sans-serif; padding: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #0f172a; padding-bottom: 15px; margin-bottom: 20px;">
            <div>
                ...
                <div style="font-size: 11px; color: #475569; margin-top: 5px;">RV NEGÓCIOS E COMPANHIA | CNPJ: 61.893.912/0001-24</div>
                <div style="font-size: 11px; color: #475569;">Rua Mineiros, 530 | Jataí - GO | (64) 99981-5852</div>
            </div>
            <div style="text-align: right;">
                <h2 style="margin: 0; font-size: 16px; font-weight: 900;">ORDEM DE COMPRA Nº ${o.id}</h2>
                <div style="font-size: 11px;">Emissão: ${formatDate(o.data)}</div>
                <div style="font-size: 11px;">Fase: ${o.fase_obra || '-'}</div>
            </div>
        </div>
        <div style="display: flex; gap: 20px; margin-bottom: 20px;">
            <div style="flex: 1; border: 1px solid #000; padding: 10px; font-size: 12px;">
                <strong>FORNECEDOR</strong><br>${forn.nome}<br>Doc: ${forn.documento || '-'}<br>Tel: ${forn.telefone || '-'}
            </div>
            <div style="flex: 1; border: 1px solid #000; padding: 10px; font-size: 12px;">
                <strong>DESTINO</strong><br>${obra.nome}<br>${obra.endereco || '-'}
            </div>
        </div>
        ...
        <div style="display: flex; justify-content: space-between;">
            <div style="border: 1px dashed #cbd5e1; padding: 10px; font-size: 11px; width: 55%;">
                <strong>Obs:</strong> ${justObs}
            </div>
        ...
        <div style="text-align: center; margin-top: 60px; font-size: 11px;">
            _________________________________<br>ASSINATURA RESPONSÁVEL
        </div>
    </div>`;
...
function buildHTMLRomaneio(id) {
    ...
    return `<div style="font-family: 'Helvetica', sans-serif; padding: 15px; width: 100%; border: 1px solid #000; min-height: 27.5cm;">
        <table width="100%" style="border-bottom: 2px solid #000; padding-bottom: 15px; margin-bottom: 20px;">
            <tr>
                ...
                <td valign="middle" style="padding-left: 15px;">
                    ...
                    <div style="font-size: 11px; color: #475569; margin-top: 5px;">CNPJ: 61.893.912/0001-24</div>
                    <div style="font-size: 11px; color: #475569;">Rua Mineiros, 530 | Jataí - GO | (64) 99981-5852</div>
                </td>
        ...
        <table width="100%" style="margin-bottom: 20px;">
            <tr>
                <td width="48%" valign="top" style="border: 1px solid #000; padding: 0;">
                    ...
                    <div style="padding: 10px; font-size: 12px;"><strong>${forn.nome}</strong><br>Doc: ${forn.documento || '-'}<br>Tel: ${forn.telefone || '-'}</div>
                </td>
                <td width="4%"></td>
                <td width="48%" valign="top" style="border: 1px solid #000; padding: 0;">
                    ...
                    <div style="padding: 10px; font-size: 12px;"><strong>${obra.nome}</strong><br>Endereço: ${obra.endereco || '-'}</div>
                </td>
            </tr>
        </table>
        ...
        <table width="100%" style="margin-top: 80px;">
            <tr>
                <td align="center" width="45%">
                    <div style="border-top: 1px solid #000; padding-top: 5px; font-size: 11px; font-weight: bold;">ASSINATURA MOTORISTA / RETIRADA</div>
                </td>
                <td width="10%"></td>
                <td align="center" width="45%">
                    <div style="border-top: 1px solid #000; padding-top: 5px; font-size: 11px; font-weight: bold;">ASSINATURA RECEBIMENTO (OBRA)</div>
                </td>
            </tr>
        </table>
    </div>`;
...
async function downloadOCPDF(id) {
    ...
    let fullObs = o.observacao || '';
    let justObs = fullObs;
    if (fullObs.includes(' | Obs: ')) justObs = fullObs.split(' | Obs: ')[1];

    let total = 0;
    const bodyTabela = [
        [{ text: 'Produto', style: 'tableHeader' }, { text: 'Qtd', style: 'tableHeader', alignment: 'center' }, { text: 'Unit.', style: 'tableHeader', alignment: 'right' }, { text: 'Total', style: 'tableHeader', alignment: 'right' }],
        ...logsOC.map(i => {
            const sub = parseFloat(i.valor_total);
            total += sub;
            const unit = sub / parseFloat(i.quantidade);
            return [i.produto_nome, { text: String(i.quantidade), alignment: 'center' }, { text: formatMoney(unit), alignment: 'right' }, { text: formatMoney(sub), alignment: 'right' }];
        })
    ];
    ...
    const docDefinition = {
        ...
        content: [
            { table: { widths: ['*', 'auto'], body: [[
                [
                    { text: 'RV NEGÓCIOS E COMPANHIA', bold: true, fontSize: 14, color: '#1d4ed8' },
                    { text: 'CNPJ: 61.893.912/0001-24', fontSize: 9, color: '#475569', lineHeight: 1.3 },
                    { text: 'Rua Mineiros, 530 | Jataí - GO | (64) 99981-5852', fontSize: 9, color: '#475569', lineHeight: 1.3, margin: [0, 10, 0, 0] }
                ],
                { text: [{ text: 'ORDEM DE COMPRA\n', fontSize: 13, bold: true }, { text: `Nº: ${o.id}\n`, fontSize: 9 }, { text: `Emissão: ${formatDate(o.data)}\n`, fontSize: 9 }, { text: `Fase: ${o.fase_obra || '-'}`, fontSize: 9 }], alignment: 'right', margin: [0, 10, 0, 0] }
            ]] }, layout: 'noBorders', margin: [0, 0, 0, 10] },
            { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 2 }], margin: [0, 0, 0, 20] },
            { columns: [
                { width: '48%', table: { widths: ['*'], body: [[{ text: 'DADOS DO FORNECEDOR', fillColor: '#f1f5f9', bold: true, fontSize: 8, margin: [4, 4, 4, 4] }], [{ text: `${forn.nome}\nDoc: ${forn.documento || '-'}\nTel: ${forn.telefone || '-'}`, fontSize: 9, margin: [6, 8, 6, 8] }]] }, layout: { hLineWidth: () => 1, vLineWidth: () => 1, hLineColor: () => '#000000', vLineColor: () => '#000000' } },
                { width: '4%', text: '' },
                { width: '48%', table: { widths: ['*'], body: [[{ text: 'DESTINO (OBRA)', fillColor: '#f1f5f9', bold: true, fontSize: 8, margin: [4, 4, 4, 4] }], [{ text: `${obra.nome}\nEndereço: ${obra.endereco || '-'}`, fontSize: 9, margin: [6, 8, 6, 8] }]] }, layout: { hLineWidth: () => 1, vLineWidth: () => 1, hLineColor: () => '#000000', vLineColor: () => '#000000' } }
            ], margin: [0, 0, 0, 20] },
            { table: { headerRows: 1, widths: ['*', 'auto', 'auto', 'auto'], body: bodyTabela }, layout: { hLineWidth: function(i, node) { return (i === 0 || i === node.table.body.length) ? 0 : 1; }, vLineWidth: () => 0, hLineColor: () => '#000000', paddingLeft: () => 4, paddingRight: () => 4 }, margin: [0, 0, 0, 20] },
            { columns: [
                { width: '60%', table: { widths: ['*'], body: [[{ text: [{ text: 'Observações:\n', fontSize: 9, bold: true }, { text: justObs, fontSize: 9 }], margin: [6, 6, 6, 25] }]] }, layout: { hLineWidth: () => 1, vLineWidth: () => 1, hLineColor: () => '#cbd5e1', vLineColor: () => '#cbd5e1', hLineStyle: () => ({ dash: { length: 4, space: 4 } }), vLineStyle: () => ({ dash: { length: 4, space: 4 } }) } },
...
async function executarTransferenciaPortal() {
    ...
    if (senhaDigitada !== senhaSecreta) return showToast("Senha Incorreta! Transferência abortada.", true);

    showLoading(true);
    const itensOC = STATE.logs.filter(l => String(l.id) === String(ocId) && (l.tipo === 'oc_pendente' || l.tipo === 'compra'));

    if (!itensOC.length) { showLoading(false); return showToast("Erro: Itens da O.C. não encontrados.", true); }

    try {
        const { data: ultimosLogs, error: errLogs } = await sb.from('logs').select('id').order('id', { ascending: false }).limit(1);
        let nextId = ultimosLogs && ultimosLogs.length > 0 ? parseInt(ultimosLogs[0].id) + 1 : 1;
        const dataHojeTexto = new Date().toLocaleDateString('pt-BR');

        const insertsRVPortal = itensOC.map(item => ({
            uid: crypto.randomUUID(), id: nextId, tipo: 'orcamento',
            produto_nome: item.produto_nome, quantidade: parseFloat(item.quantidade),
            data: dataHojeTexto, observacao: item.observacao || 'Gerado automaticamente via RV Negócios',
            valor_total: parseFloat(item.valor_total), cliente_nome: 'RV Negócios',
            forma_pagamento: '', status: 'ATIVO', status_entrega: '', qtd_entregue: 0,
            desconto: 0, status_financeiro: 'PENDENTE', vencimento: '', valor_pago: 0, endereco_entrega: ''
        }));
