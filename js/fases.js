// =============================================
//   RV NEGÓCIOS - js/fases.js
//   CRUD de Fases de Obra
// =============================================

function renderFases() {
    const term = document.getElementById('fase-search').value.toLowerCase();
    const list = document.getElementById('fases-list');
    const fil = STATE.fases.filter(f =>
        f.nome.toLowerCase().includes(term) || String(f.ordem).includes(term)
    );
...
async function saveFase(e) {
    ...
    payload.id = isNew ? crypto.randomUUID() : document.getElementById('fas-id').value;
