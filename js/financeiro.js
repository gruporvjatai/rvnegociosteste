...
function renderFinance() {
    ...
    let logsFilt = STATE.logs.filter(l => l.tipo !== 'oc_pendente');
    if (obraFilter) logsFilt = logsFilt.filter(l => String(l.obra_id) === String(obraFilter));

    let totDesp = 0, totRec = 0;
    const valorContratoFiltrado = obraFilter ? parseFloat(STATE.obras.find(o => String(o.id) === String(obraFilter))?.valor_contrato || 0) : 0;

    // Lista de Despesas
    const listaDesp = document.getElementById('fin-expenses-list');
    ...
    listaDesp.innerHTML = despArr.map(l => {
        ...
        const forn = STATE.fornecedores.find(x => x.id == l.fornecedor_id);
        const extraInfo = l.tipo === 'compra' && forn ? `<div class="text-[9px] text-slate-500 font-bold uppercase mt-0.5"><i data-lucide="truck" class="inline w-3 h-3"></i> ${forn.nome}</div>` : '';

        const btnAcoes = !isPago ? `
            <button onclick="baixarLog('${l.uid}')" class="bg-blue-700 hover:bg-blue-800 text-white p-1.5 rounded shadow flex items-center gap-1 text-[10px] font-bold" title="Confirmar Pagamento"><i data-lucide="check-circle" class="w-3.5 h-3.5"></i> PAGAR</button>
            ${l.tipo === 'despesa' ? `<button onclick="excluirLancamento('${l.uid}')" class="bg-slate-200 hover:bg-slate-300 text-slate-600 p-1.5 rounded shadow" title="Cancelar"><i data-lucide="trash" class="w-3.5 h-3.5"></i></button>` : ''}
            ${l.tipo === 'compra' ? `
                <button onclick="baixarLogOC('${l.id}')" class="bg-slate-700 hover:bg-slate-800 text-white p-1.5 rounded shadow text-[10px] font-bold" title="Pagar toda a O.C."><i data-lucide="check-check" class="w-3.5 h-3.5"></i></button>
                <button onclick="excluirLancamentoOC('${l.id}')" class="bg-slate-200 hover:bg-slate-300 text-slate-600 p-1.5 rounded shadow" title="Cancelar O.C."><i data-lucide="trash" class="w-3.5 h-3.5"></i></button>
            ` : ''}
        ` : `
            <span class="text-[10px] font-bold text-blue-700 bg-blue-100 px-2 py-1 rounded">PAGO</span>
            <button onclick="estornarBaixa${l.tipo === 'compra' ? 'OC' : ''}('${l.tipo === 'compra' ? l.id : l.uid}')" class="bg-slate-200 hover:bg-slate-300 text-slate-600 p-1.5 rounded shadow" title="Estornar Baixa"><i data-lucide="rotate-ccw" class="w-3 h-3"></i></button>
        `;

        return `<tr class="border-b ${isPago ? 'bg-green-50/20' : 'bg-red-50/20'} transition">
        ...
        return `<tr class="border-b ${isPago ? 'bg-green-50' : 'bg-orange-50/20'} transition">
            ...
            <td class="p-3 text-right flex items-center justify-end gap-1">
                ${!isPago ? `
                    ...
                    <button onclick="excluirLancamento('${l.uid}')" class="bg-slate-200 hover:bg-slate-300 text-slate-600 p-1.5 rounded shadow" title="Cancelar"><i data-lucide="trash" class="w-3.5 h-3.5"></i></button>
                ` : `
    ...
    if (obraFilter) {
        cardsContainer.innerHTML = `
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-slate-800 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">Saldo Contrato</p><h3 class="text-xl font-black text-slate-800 mt-1">${formatMoney(valorContratoFiltrado)}</h3></div>
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-green-500 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">Medições (Rec.)</p><h3 class="text-xl font-black text-green-700 mt-1">${formatMoney(totRec)}</h3></div>
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-orange-500 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">A Receber</p><h3 class="text-xl font-black text-orange-600 mt-1">${formatMoney(valorContratoFiltrado - totRec)}</h3></div>
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-red-500 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">Despesas (Pagas)</p><h3 class="text-xl font-black text-red-600 mt-1">${formatMoney(totDesp)}</h3></div>
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-blue-600 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">Caixa da Obra</p><h3 class="text-xl font-black text-blue-700 mt-1">${formatMoney(totRec - totDesp)}</h3></div>
        `;
    } else {
        cardsContainer.innerHTML = `
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-red-500 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">Total de Custos GERAIS</p><h3 class="text-2xl font-black text-red-600 mt-1">${formatMoney(totDesp)}</h3></div>
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-green-500 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">Total Medições / Receitas GERAIS</p><h3 class="text-2xl font-black text-green-700 mt-1">${formatMoney(totRec)}</h3></div>
            <div class="bg-white p-4 rounded-xl border-l-4 border-l-blue-600 shadow-sm"><p class="text-slate-500 text-xs font-bold uppercase">Saldo Geral de Caixa</p><h3 class="text-2xl font-black text-blue-700 mt-1">${formatMoney(totRec - totDesp)}</h3></div>
        `;
    }
    lucide.createIcons();
}
...
async function estornarBaixa(uid) {
    if (!confirm("Tem certeza que deseja ESTORNAR a baixa? O status voltará para PENDENTE.")) return;
    showLoading(true);
...
async function baixarLogOC(id) {
    if (!confirm("Confirmar a baixa financeira desta O.C.? Todos os itens serão marcados como pagos.")) return;
    showLoading(true);
...
async function excluirLancamentoOC(id) {
    if (!confirm("Deseja CANCELAR esta O.C. no Financeiro?")) return;
    showLoading(true);
...
async function saveExpense() {
    ...
    let obsArr = [`Tipo: ${tipoLancamento}`];
