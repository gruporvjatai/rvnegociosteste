// ====== TERCEIRIZADOS (EMPREITEIROS POR METRAGEM) ======
...
function renderTercProdList(id) {
    ...
    if (prod.length === 0) {
        document.getElementById('prod-terc-list-body').innerHTML = `<tr><td colspan="4" class="p-6 text-center text-slate-400 font-medium text-xs">Nenhuma medição neste mês.</td></tr>`;
    }
...
async function baixarPagamentoTerc(tercId) {
    ...
    if (!confirm(`Deseja dar baixa (Marcar como PAGO) em TODAS AS MEDIÇÕES PENDENTES de ${mes}/${ano} para ${t.nome}?`)) return;
    ...
    if (error) {
        showLoading(false);
        return showToast("Erro ao baixar: " + error.message, true);
    }
...
function imprimirRelatorioTerc() {
    ...
    let labelFiltro = pagFiltro === 'pendente' ? 'VALORES PENDENTES A PAGAR' : (pagFiltro === 'pago' ? 'VALORES JÁ PAGOS' : 'PRODUÇÃO TOTAL DO MÊS');
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #1e293b; padding: 10px;">
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 3px solid #1d4ed8; padding-bottom: 15px; margin-bottom: 25px;">
                ...
                <div style="text-align: right;">
                    ...
                    <p style="margin: 3px 0 0 0; font-size: 13px; color: #1d4ed8; font-weight: bold;">Competência: ${mes}/${ano}</p>
        ...
        if (metrosConsiderados > 0) {
            ...
            htmlRelatorio += `
                <tr style="font-size: 12px; border-bottom: 1px solid #e2e8f0;">
                    ...
                    <td style="padding: 10px; text-align: center; font-weight: 800; color: #0f172a;">${metrosConsiderados.toFixed(2)} m</span></td>
